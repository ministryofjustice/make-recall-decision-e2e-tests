...
import { selectRandomRadio } from "../../utils/inputs/radios";
...
import { textInputRandomText } from "../../utils/inputs/text";
import { textAreaRandomText } from "../../utils/inputs/textArea";
import { createRecommendationRequest, initialRecomendationRequest, postPORecallRecommendationRequest, recordDecisionRecommendationRequest, spoSignatureRequestStatusRequest, startRecallRecommendationStatusRequest } from "./ppcs.requests";
...
Then('the user proceeds to book a recall', () => {
    const editText = 'Edit'
    ...
    cy.clickLinkById('edit-custodytype', editText)
    cy.pageHeading().should('equal', 'Edit custody type')
    selectRandomOption('#custodyType', true)
    cy.clickButton('Continue')
...
Then('the booking reports successfully sent to PPUD', () => {
    cy.pageHeading().should('contain', 'Your recall booking')
})