...
function selectVulnerabilitiesWhenRiskToSelfFlagEnabled(
  ...
  cy.pageHeading().should(
    'equal',
    `Consider if you think this recall could affect any vulnerabilities or needs ${offenderName} may have.`
  )
  cy.logPageTitle(
    `Consider if you think this recall could affect any vulnerabilities or needs ${offenderName} may have.`
  )
...
function completeVulnerabilitiesSection(partADetails: Record<string, string>, offenderName: string) {
  ...
  cy.clickLink('Could recall affect vulnerabilities or needs?')
...
When('PO downloads Part A document for Preview', function () {
  cy.clickLink('Download preview of Part A')
})
